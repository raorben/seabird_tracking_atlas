...
sta_quickplot<-function(bbgroups,
  ...
  for (h in 1:length(names(bbgroups))){
    ...
    tracks_seg_df$STA_id<-as.factor(tracks_seg_df$STA_id)
    B<-ggplot() +
      geom_polygon(data=w2hr_sub,aes((long),lat,group=group),fill="black",color="grey60",size=0.1)+
      geom_polygon(data=states_sub,aes((long),lat,group=group),fill="black",color="grey60",size=0.1)+
      geom_path(data=tracks_seg_df%>%
                  filter(timegrp==grp.ids[[h]]),
                aes(x=lon1,y=lat1,group=seg_id,color=STA_id),size=0.4)+
    ...
    ggsave(plotAB,filename = paste0(dir,"species/",sp,"/",grp.ids[h],"_",sp,"_",clipper_list$clipperName,".png"),width=10,dpi = 300)
  ...
  sta_quickplot<-function(bbgroups,
    ...
    for (h in 1:length(names(bbgroups))){
      ...
      A<-ggplot() +
        ...
        xlab("Longitude")+
        ylab("Latitude")+
        coord_fixed(ratio=1.7,xlim = c(-130,-118),ylim=c(30,49))+
